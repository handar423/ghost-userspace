...
// 只要数据传输在中间态，就会被忽略掉
void FlexOrchestrator::RefreshSchedParam(
  ...
  FlexSchedParams* sp = &cachedsids_[sid];
...
// have changed.
void FlexOrchestrator::RefreshAllSchedParams(
...
void FlexOrchestrator::RefreshSchedParams(
    const SchedCallbackFunc& SchedCallback) {
  int updatedIndex;
...
void FlexOrchestrator::GetSchedParams(Gtid gtid,
...
void FlexOrchestrator::DumpSchedParams() const {
  fprintf(stderr, "FlexSchedParams:\n");
...
void FlexOrchestrator::MakeEngineRunnable(const FlexSchedParams* sp) {
